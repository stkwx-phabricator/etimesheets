...
class REST_Controller_Plugin_RestHandler extends Zend_Controller_Plugin_Abstract
{
    ...
    {
        /**
         * TODO: probably should be in application.ini?
         * Default for Google's Page Speed recomendations
         */
        // Cache for...
        $time = strtotime('+1 week');
        // Calculation
        $secs = $time - time();
        $today = gmdate('D, d M Y H:i:00');
        $expire = gmdate('D, d M Y H:i:s', $time);
        // Cache headers
        $this->_response->setHeader('Expires', "$expire GMT");
        $this->_response->setHeader('Last-Modified', "$today GMT");
        $this->_response->setHeader('Cache-Control', "public, no-transform, max-age=$secs, s-maxage=$secs");
        // Cross-Origin Resource Sharing (CORS)
       // $this->_response->setHeader('Access-Control-Max-Age', $secs);
        //$this->_response->setHeader('Access-Control-Allow-Origin', '*.ge.com');
